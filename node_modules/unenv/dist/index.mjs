...
const defineEnv = (opts = {}) => {
	...
	if (opts.resolve) resolveEnvPaths(env, presets, opts);
	return {
		env,
		presets
	};
...
function resolveEnvPaths(env, presets, opts = {}) {
	...
	for (let i = 0; i < env.polyfill.length; i++) env.polyfill[i] = _resolve(env.polyfill[i]);
